...
nodes = pl.read_csv(
    ...
    has_header=False,
    new_columns=["x", "y", "z"],
).with_row_index(name="node", offset=1).with_columns(
    pl.col("x", "y", "z") / 10
)
...
elements_with_euclidean_distances = (
    elements_with_coordinates
    .join(field_points, how="cross")
    .select(
...
velocities_per_frequency = (
    velocity
    .with_columns(
        (pl.col("frequency") * 2 * np.pi).alias("omega"),
        ((pl.col("frequency") * 2 * np.pi) / c0).alias("k")
    )
    .sort(by="frequency")
)
...
rayleigh_integral_per_frequency = (
    elements_with_euclidean_distances
    .join(velocities_per_frequency, how="cross")
...
velocities_array
far_field_pressure = -1j*0.5*rho0*c0*velocities_array["u"] * velocities_array["k"] * a * (a/r) * np.exp(1j*velocities_array["k"]*r)
...
ax.semilogx(pressure_per_frequency.select("frequency"), pressure_per_frequency.select("Lp"), label="Simulado", alpha=0.5)
ax.legend()
